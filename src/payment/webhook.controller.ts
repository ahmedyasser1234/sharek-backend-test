...
export class WebhookController {
  ...
  @Post('stripe')
  handleStripe(@Body() body: unknown, @Headers('stripe-signature') signature: string): void {
    try {
  ...
  @Post('hyperpay')
  handleHyperPay(@Body() body: { transaction: HyperPayTransaction }): void {
  ...
  @Post('paytabs')
  handlePayTabs(@Body() body: PayTabsWebhookBody): void {
  ...
  @Post('tap')
  handleTap(@Body() body: TapWebhookBody): void {
  ...
  @Post('geidea')
  handleGeidea(@Body() body: GeideaWebhookBody): void {
    if (body.status === 'PAID') {
  ...
  @Post('stcpay')
  handleSTCPay(@Body() body: { transaction: STCPayTransaction }): void {
    const { transaction } = body;
    if (transaction?.status === 'SUCCESS') {
