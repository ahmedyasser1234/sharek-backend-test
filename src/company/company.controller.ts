...
export class CompanyController {
  ...
  oauthLogin(@Body('provider') provider: 'google' | 'facebook' | 'linkedin', @Body('token') token: string) {
    if (!provider || !token) throw new BadRequestException('مزود الخدمة والتوكن مطلوبين');
  ...
  verifyCode(@Body() body: { email: string; code: string }) {
    if (!body.email || !body.code) throw new BadRequestException('الإيميل والكود مطلوبين');
  ...
  async requestPasswordReset(@Body('email') email: string) {
    if (!email) throw new BadRequestException('الإيميل مطلوب');
    this.logger.log(`🔐 طلب كود إعادة تعيين كلمة المرور للبريد: ${email}`);
  ...
  refresh(@Req() req: Request) {
    ...
    if (!refreshToken) throw new BadRequestException('Refresh token مطلوب في الهيدر');
  ...
  async getProfile(@Req() req: CompanyRequest) {
    if (!req.user?.companyId) throw new UnauthorizedException('Unauthorized access');
    const companyId = req.user.companyId;
    const company = await this.companyService.findById(companyId);
    const currentSub = company.subscriptions?.sort((a, b) => b.startDate.getTime() - a.startDate.getTime())[0];
  ...
  update(@Param('id') id: string, @Body() dto: UpdateCompanyDto, @UploadedFile() logo: Express.Multer.File) {
